	...
	async.mapSeries(urlCityList, (urlCity, cb2)=>{
		...
		request(url, (error, response, html)=>{
        	...
        	restaurantsWithCityList.push({
        		city: restaurantsWithCityList.city,
        		urlList: tmpUrlList
        	});
	...
	async.mapSeries(urlRestaurantWithCityList, (urlRestaurantWithCity, cb2)=>{
		...
		async.mapSeries(urlList, (url, cb3)=>{
			request(url, (error, response, html)=>{
		        ...
		        let latLon = $('script[type="text/javascript"]').each((i, script)=>{
		        	script = $(script).text();
		        	if(script.includes('LatLng')){
		        		let latLon = script.substring(script.lastIndexOf("LatLng(")+1,script.lastIndexOf("LatLng(")+50).match(/[+-]?([0-9]*[.])?[0-9]+/g);
